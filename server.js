...
// âââ Meta API Proxy âââââââââââââââââââââââââââââââââââââââ
...
// âââ HTML App âââââââââââââââââââââââââââââââââââââââââââââ
...
<div id="loginPage" class="login-wrap">
  <div class="login-card">
    <div class="login-logo">â META<span>ADS</span> <span style="color:var(--muted);font-size:.7rem">PRO</span></div>
    <div class="login-sub">××ª×××¨ ×× ×××× ××§××¤××× ×× ×©××</div>
    <div id="loginErr" class="login-err">â ï¸ ×©× ××©×ª××© ×× ×¡××¡×× ×©×××××</div>
    <div class="form-group">
      <label class="form-label">×©× ××©×ª××©</label>
      <input type="text" class="form-input" id="loginUser" placeholder="×©× ××©×ª××©" style="direction:rtl;text-align:right" onkeydown="if(event.key==='Enter')document.getElementById('loginPass').focus()"/>
    </div>
    <div class="form-group">
      <label class="form-label">×¡××¡××</label>
      <input type="password" class="form-input" id="loginPass" placeholder="â¢â¢â¢â¢â¢â¢â¢â¢" onkeydown="if(event.key==='Enter')login()"/>
    </div>
    <button class="btn-login" onclick="login()" id="loginBtn">ð ××ª×××¨</button>
...
<header class="header">
  <div class="logo">â META<span>ADS</span></div>
  ...
  <div class="header-right">
    ...
    <button class="btn btn-sm btn-ghost" onclick="refreshAll()">ð</button>
    <button class="btn btn-sm btn-ghost" onclick="logout()">××¦×××</button>
  </div>
</header>

<div class="date-bar">
  <button class="date-btn active" onclick="setDP('today',this)">××××</button>
  <button class="date-btn" onclick="setDP('yesterday',this)">××ª×××</button>
  <button class="date-btn" onclick="setDP('last_7d',this)">7 ××××</button>
  <button class="date-btn" onclick="setDP('last_14d',this)">14 ××××</button>
  <button class="date-btn" onclick="setDP('last_30d',this)">30 ××××</button>
  <button class="date-btn" onclick="setDP('last_month',this)">××××© ×©×¢××¨</button>
  ...
  <button class="btn btn-sm btn-ghost" onclick="setCustomDate()">××¤×¢×</button>
</div>
...
<div class="layout">
  <nav class="sidebar">
    <div class="nav-section">
      ...
      <div class="nav-item active" onclick="nav('dashboard',this)">ð ××©×××¨×</div>
      <div class="nav-item" onclick="nav('campaigns',this)">ð ×§××¤××× ××</div>
      <div class="nav-item" onclick="nav('adsets',this)">ð¯ Ad Sets</div>
      <div class="nav-item" onclick="nav('ads',this)">ð¼ï¸ ××××¢××ª</div>
    </div>
    <div class="nav-section">
      <span class="nav-label">××××</span>
      <div class="nav-item" onclick="nav('create',this)">â ××¦××¨×</div>
      <div class="nav-item" onclick="nav('alerts',this)">ð ××ª×¨×××ª <span id="alertBadge" style="display:none;background:var(--warn);color:#fff;border-radius:10px;font-size:.62rem;padding:1px 5px;margin-right:4px"></span></div>
      <div class="nav-item" onclick="nav('ai',this)">ð¤ AI</div>
      <div class="nav-item" onclick="nav('export',this)">ð¥ ×××¦××</div>
    </div>
  </nav>
  ...
  <main class="main">
    ...
    <div class="page active" id="page-dashboard">
      ...
      <div class="chart-card">
        <div class="chart-header">
          <div class="chart-title">ð ×××¦××¢×× ××××¨× ×××</div>
          <div class="chart-toggles">
            <button class="chart-toggle active-spend" onclick="toggleMetric('spend',this)">×××¦××</button>
            <button class="chart-toggle" onclick="toggleMetric('clicks',this)">×§×××§××</button>
      ...
      <div class="section-header">
        <div class="section-title">ð¥ ×§××¤××× ×× ×¤×¢××××</div>
        <button class="btn btn-sm btn-accent" onclick="nav('campaigns',document.querySelectorAll('.nav-item')[1])">×××× â</button>
      </div>
    ...
    <div class="page" id="page-campaigns">
      <div class="page-title">ð ×§××¤××× ××</div>
      <div class="page-sub">×××¥ ×¢× ×××ª×¨×ª ×××××</div>
      <div class="section-header">
        <div style="display:flex;gap:.4rem;flex-wrap:wrap">
          <button class="btn btn-sm btn-ghost" onclick="loadCampaigns()">ð ×¨×¢× ×</button>
          <button class="btn btn-sm btn-accent" onclick="nav('create',document.querySelectorAll('.nav-item')[4])">â ×××©</button>
        </div>
        <input class="search-input" placeholder="ð ×××¤××©..." oninput="filterTbl('campTbl',this.value)"/>
      </div>
      <div class="table-wrap" id="campTbl"><div class="loading"><div class="spinner"></div></div></div>
    </div>

    <div class="page" id="page-adsets">
      <div class="page-title">ð¯ Ad Sets</div>
      <div class="page-sub">×§×××¦××ª ××××¢××ª</div>
      <div class="section-header">
        <button class="btn btn-sm btn-ghost" onclick="loadAdsets()">ð ×¨×¢× ×</button>
        <input class="search-input" placeholder="ð ×××¤××©..." oninput="filterTbl('adsetTbl',this.value)"/>
      </div>
      <div class="table-wrap" id="adsetTbl"><div class="loading"><div class="spinner"></div></div></div>
    </div>

    <div class="page" id="page-ads">
      <div class="page-title">ð¼ï¸ ××××¢××ª</div>
      <div class="page-sub">×× ×××××¢××ª</div>
      <div class="section-header">
        <button class="btn btn-sm btn-ghost" onclick="loadAds()">ð ×¨×¢× ×</button>
        <input class="search-input" placeholder="ð ×××¤××©..." oninput="filterTbl('adsTbl',this.value)"/>
      </div>
      <div class="table-wrap" id="adsTbl"><div class="loading"><div class="spinner"></div></div></div>
    </div>

    <div class="page" id="page-create">
      <div class="page-title">â ×§××¤××× ×××©</div>
      <div class="page-sub">××××¨ ××ª ×¤×¨×× ××§××¤×××</div>
      <div class="table-wrap" style="padding:1.4rem;max-width:540px">
        <div id="createErr" class="error-box" style="display:none">â ï¸ <span id="createErrMsg"></span></div>
        <div id="createOk" class="success-box" style="display:none">â ××§××¤××× × ××¦×¨ ×××¦×××!</div>
        <div class="form-group"><label class="form-label">×©×</label><input type="text" class="form-input" id="campName" style="direction:rtl;text-align:right" placeholder="×©× ××§××¤×××..."/></div>
        <div class="form-group"><label class="form-label">×××¨×</label>
          <select class="form-input" id="campObj">
            <option value="OUTCOME_TRAFFIC">×ª× ××¢×</option>
            <option value="OUTCOME_ENGAGEMENT">××¢××¨×××ª</option>
            <option value="OUTCOME_LEADS">×××××</option>
            <option value="OUTCOME_SALES">××××¨××ª</option>
            <option value="OUTCOME_APP_PROMOTION">××¤×××§×¦××</option>
            <option value="OUTCOME_AWARENESS">××××¢××ª</option>
          </select>
        </div>
        <div class="form-group"><label class="form-label">×ª×§×¦×× ×××× (âª)</label><input type="number" class="form-input" id="campBudget" placeholder="100" min="1" style="direction:ltr;text-align:left"/></div>
        <div class="form-group"><label class="form-label">×¡××××¡</label>
          <select class="form-input" id="campStatus"><option value="PAUSED">×¢×¦××¨ (×××××¥)</option><option value="ACTIVE">×¤×¢××</option></select>
        </div>
        <div style="display:flex;gap:.6rem">
          <button class="btn btn-primary" onclick="createCamp()" id="createBtn" style="flex:1">ð ×¦××¨ ×§××¤×××</button>
          <button class="btn btn-ghost btn-sm" onclick="nav('campaigns',document.querySelectorAll('.nav-item')[1])">×××××</button>
        </div>
      </div>
    </div>

    <div class="page" id="page-alerts">
      <div class="page-title">ð ××ª×¨×××ª ×××××ª</div>
      <div class="page-sub">AI ×©×¡××¨×§ ××××× ××¢×××ª</div>
      <div class="section-header"><button class="btn btn-sm btn-ghost" onclick="runAlerts()">ð ×¡×¨××§</button></div>
      <div id="alertsList"><div class="loading"><div class="spinner"></div></div></div>
    </div>

    <div class="page" id="page-ai">
      <div class="page-title">ð¤ AI Assistant</div>
      <div class="page-sub">×©×× ×©××××ª ××¢××¨××ª ×¢× ××§××¤××× ××</div>
      <div class="ai-box">
        <div class="ai-header">â¨ Claude ×× ×ª× ××ª ×× ×ª×× ×× ×©××</div>
        <div class="ai-input-row">
          <input type="text" class="ai-input" id="aiQ" placeholder="××××××: ×××× ×§××¤××× ×× ××¢×¦××¨?" onkeydown="if(event.key==='Enter')askAI()"/>
          <button class="btn btn-accent btn-sm" onclick="askAI()" id="aiBtn">×©××</button>
        </div>
        <div class="ai-response" id="aiResp"></div>
        <div class="ai-suggestions">
          <button class="btn btn-ghost btn-sm" onclick="setQ('×××× ×§××¤××× ×× ×× ×××××× ×ª××¦×××ª?')">×§××¤××× ×× ×××©××</button>
          <button class="btn btn-ghost btn-sm" onclick="setQ('×× ×-CPA ×××××¦×¢ ×©××?')">CPA ××××¦×¢</button>
          <button class="btn btn-ghost btn-sm" onclick="setQ('×××× ×§××¤××× ××× ×××?')">××§××¤××× ××××</button>
          <button class="btn btn-ghost btn-sm" onclick="setQ('×ª× ×× ××××¦××ª ××©××¤××¨')">××××¦××ª</button>
        </div>
      </div>
    </div>

    <div class="page" id="page-export">
      <div class="page-title">ð¥ ×××¦×× ×××××ª</div>
      <div class="page-sub">×××¨× ××©×ª×£ ×¢× ××§××××ª</div>
      <div class="export-grid">
        <div class="export-card" onclick="exportCSV()"><div class="export-icon">ð</div><div class="export-title">CSV ×××§×¡×</div><div class="export-desc">×× × ×ª×× × ××§××¤××× ××</div></div>
        <div class="export-card" onclick="exportJSON()"><div class="export-icon">ðï¸</div><div class="export-title">JSON ×××××</div><div class="export-desc">××¢×××× × ××¡×£</div></div>
        <div class="export-card" onclick="exportPDF()"><div class="export-icon">ð</div><div class="export-title">××× PDF</div><div class="export-desc">××©×××× ×××§××××ª</div></div>
        <div class="export-card" onclick="copyShare()"><div class="export-icon">ð</div><div class="export-title">××¢×ª×§ ××©××ª××£</div><div class="export-desc">××××¡××¤ / ××××</div></div>
      </div>
...
<div class="modal-overlay" id="editModal" style="display:none">
  <div class="modal">
    <div class="modal-header"><div class="modal-title">âï¸ ×¢×¨×××ª ×§××¤×××</div><button class="close-btn" onclick="closeModal()">â</button></div>
    <div id="editContent"></div>
    <div class="modal-footer">
      <button class="btn btn-ghost btn-sm" onclick="closeModal()">×××××</button>
      <button class="btn btn-accent btn-sm" onclick="saveEdit()">ð¾ ×©×××¨</button>
    </div>
  </div>
</div>
...
const fmtM=n=>'âª'+parseFloat(n||0).toFixed(2);
...
async function login() {
  ...
  btn.textContent = 'â³ ××ª×××¨...'; btn.disabled = true;
  ...
  try {
    ...
    btn.textContent = 'ð ××ª×××¨'; btn.disabled = false;
  }
}
...
// âââ API via Server Proxy âââââââââââââââââââââââââââââââââ
...
// âââ DATE âââââââââââââââââââââââââââââââââââââââââââââââââ
...
async function loadDashboard(){
  ...
  try{
    ...
    const chg=(cur,prev)=>{if(!prev||prev==0)return'';const pct=((cur-prev)/prev*100).toFixed(1);return\`<span class="stat-change \${pct>=0?'up':'down'}">\${pct>=0?'â':'â'}\${Math.abs(pct)}%</span>\`;};
    document.getElementById('statsGrid').innerHTML=\`
      <div class="stat-card"><div class="stat-label">×××¦××</div><div class="stat-value" style="color:var(--accent)">\${fmtM(d.spend)}</div><div class="stat-sub">×ª×§××¤× × ×××¨×ª</div>\${chg(parseFloat(d.spend),parseFloat(p.spend))}</div>
      <div class="stat-card"><div class="stat-label">×××¤×¢××ª</div><div class="stat-value" style="color:var(--orange)">\${fmt(d.impressions)}</div><div class="stat-sub">\${fmt(d.clicks)} ×§×××§××</div>\${chg(parseFloat(d.impressions),parseFloat(p.impressions))}</div>
      <div class="stat-card"><div class="stat-label">CTR</div><div class="stat-value" style="color:var(--purple)">\${fmtP(d.ctr)}</div><div class="stat-sub">×©××¢××¨ ××§××§×</div>\${chg(parseFloat(d.ctr),parseFloat(p.ctr))}</div>
      <div class="stat-card"><div class="stat-label">×¨×××©××ª</div><div class="stat-value" style="color:var(--accent2)">\${fmt(purchases)}</div><div class="stat-sub">purchase events</div></div>
      <div class="stat-card"><div class="stat-label">CPA</div><div class="stat-value" style="color:#fd79a8">\${cpa?fmtM(cpa):'-'}</div><div class="stat-sub">×¢×××ª ××¨×××©×</div></div>
      <div class="stat-card"><div class="stat-label">ROAS</div><div class="stat-value" style="color:#55efc4">\${roas?roas+'x':'-'}</div><div class="stat-sub">×ª×©××× ×¢× ×××¦××</div></div>\`;
    document.getElementById('dashSub').textContent=\`×¢××××: \${new Date().toLocaleTimeString('he-IL')}\`;
  }catch(e){document.getElementById('statsGrid').innerHTML=\`<div class="error-box" style="grid-column:1/-1">â ï¸ \${e.message}</div>\`;}
  ...
  catch(e){document.getElementById('dashCampaigns').innerHTML=\`<div class="error-box" style="margin:1rem">â ï¸ \${e.message}</div>\`;}
}
...
// âââ CAMPAIGNS ââââââââââââââââââââââââââââââââââââââââââââ
async function loadCampaigns(){
  ...
  catch(e){document.getElementById('campTbl').innerHTML=\`<div class="error-box" style="margin:1rem">â ï¸ \${e.message}</div>\`;}
}
function renderCampTable(el,data,mini){
  if(!data.length){document.getElementById(el).innerHTML='<div class="loading">××× ×§××¤××× ××</div>';return;}
  const rows=data.map(c=>{
    const ins=c.insights?.data?.[0]||{};
    const badge=c.status==='ACTIVE'?'badge-active':'badge-paused';
    const label=c.status==='ACTIVE'?'×¤×¢××':'×¢×¦××¨';
    const budget=c.daily_budget?fmtM(c.daily_budget/100)+'/×':'-';
    const purchases=(ins.actions||[]).find(a=>a.action_type==='purchase')?.value||0;
    const acts=mini?'':\`<td>\${purchases||'-'}</td><td><div class="actions">
      \${c.status==='ACTIVE'?\`<button class="btn btn-sm btn-danger" onclick="toggle('\${c.id}','PAUSED','camp')">â¸</button>\`:\`<button class="btn btn-sm btn-success" onclick="toggle('\${c.id}','ACTIVE','camp')">â¶</button>\`}
      <button class="btn btn-sm btn-ghost" onclick="openEdit('\${c.id}')">âï¸</button>
    </div></td>\`;
  ...
  const h=mini?'':\`<th>×¨×××©××ª</th><th>×¤×¢××××ª</th>\`;
  document.getElementById(el).innerHTML=\`<table><thead><tr><th>×©×</th><th>×¡××××¡</th><th>×××¨×</th><th>×ª×§×¦××</th><th>×××¦××</th><th>×××¤×¢××ª</th><th>CTR</th>\${h}</tr></thead><tbody>\${rows.join('')}</tbody></table>\`;
}
...
async function loadAdsets(){
  ...
  if(!adsetsData.length){document.getElementById('adsetTbl').innerHTML='<div class="loading">××× Ad Sets</div>';return;}
  const rows=adsetsData.map(a=>{const ins=a.insights?.data?.[0]||{};const badge=a.status==='ACTIVE'?'badge-active':'badge-paused';
    return \`<tr><td style="font-weight:600">\${a.name}</td><td><span class="badge \${badge}"><span class="dot"></span>\${a.status==='ACTIVE'?'×¤×¢××':'×¢×¦××¨'}</span></td><td>\${a.daily_budget?fmtM(a.daily_budget/100)+'/×':'-'}</td><td>\${fmtM(ins.spend)}</td><td>\${fmt(ins.impressions)}</td><td>\${fmtP(ins.ctr)}</td><td><div class="actions">\${a.status==='ACTIVE'?\`<button class="btn btn-sm btn-danger" onclick="toggle('\${a.id}','PAUSED','adset')">â¸</button>\`:\`<button class="btn btn-sm btn-success" onclick="toggle('\${a.id}','ACTIVE','adset')">â¶</button>\`}</div></td></tr>\`;
  });
  document.getElementById('adsetTbl').innerHTML=\`<table><thead><tr><th>×©×</th><th>×¡××××¡</th><th>×ª×§×¦××</th><th>×××¦××</th><th>×××¤×¢××ª</th><th>CTR</th><th>×¤×¢××××ª</th></tr></thead><tbody>\${rows.join('')}</tbody></table>\`;}
  catch(e){document.getElementById('adsetTbl').innerHTML=\`<div class="error-box" style="margin:1rem">â ï¸ \${e.message}</div>\`;}
}
...
async function loadAds(){
  ...
  if(!adsData.length){document.getElementById('adsTbl').innerHTML='<div class="loading">××× ××××¢××ª</div>';return;}
  const rows=adsData.map(a=>{const ins=a.insights?.data?.[0]||{};const badge=a.status==='ACTIVE'?'badge-active':'badge-paused';
    return \`<tr><td style="font-weight:600">\${a.name}</td><td><span class="badge \${badge}"><span class="dot"></span>\${a.status==='ACTIVE'?'×¤×¢××':'×¢×¦××¨'}</span></td><td>\${fmtM(ins.spend)}</td><td>\${fmt(ins.impressions)}</td><td>\${fmt(ins.clicks)}</td><td>\${fmtP(ins.ctr)}</td><td><div class="actions">\${a.status==='ACTIVE'?\`<button class="btn btn-sm btn-danger" onclick="toggle('\${a.id}','PAUSED','ad')">â¸</button>\`:\`<button class="btn btn-sm btn-success" onclick="toggle('\${a.id}','ACTIVE','ad')">â¶</button>\`}</div></td></tr>\`;
  });
  document.getElementById('adsTbl').innerHTML=\`<table><thead><tr><th>×©×</th><th>×¡××××¡</th><th>×××¦××</th><th>×××¤×¢××ª</th><th>×§×××§××</th><th>CTR</th><th>×¤×¢××××ª</th></tr></thead><tbody>\${rows.join('')}</tbody></table>\`;}
  catch(e){document.getElementById('adsTbl').innerHTML=\`<div class="error-box" style="margin:1rem">â ï¸ \${e.message}</div>\`;}
}
...
async function createCamp(){
  ...
  if(!name||!budget){document.getElementById('createErr').style.display='flex';document.getElementById('createErrMsg').textContent='××© ×××× ×©× ××ª×§×¦××';return;}
  ...
  finally{btn.textContent='ð ×¦××¨ ×§××¤×××';btn.disabled=false;}
}

async function runAlerts(){
  document.getElementById('alertsList').innerHTML='<div class="loading"><div class="spinner"></div><span>×¡××¨×§...</span></div>';
  try{
    ...
    (r.data||[]).forEach(c=>{
      ...
      if(spend>50&&purchases==0)alerts.push({sev:'high',icon:'ð´',title:\`\${c.name} â ×××¦×× ××× ×××¨××ª\`,desc:\`âª\${spend.toFixed(0)} ×××¦×× ××× ×¨×××©××ª. ×©×§×× ××¢×¦××¨.\`,id:c.id});
      if(ctr>0&&ctr<0.3)alerts.push({sev:'med',icon:'ð¡',title:\`\${c.name} â CTR × ××× (\${ctr.toFixed(2)}%)\`,desc:'CTR ××ª××ª ×-0.3%. ×××××¥ ××××© ×§×¨×××××××.',id:c.id});
      if(cpa>200)alerts.push({sev:'med',icon:'ð¡',title:\`\${c.name} â CPA ×××× (âª\${parseFloat(cpa).toFixed(0)})\`,desc:'×¢×××ª ×¨×××©× ×××××.',id:c.id});
    });
    ...
    hi.length?(bar.style.display='block',bar.innerHTML=hi.map(a=>\`<div class="alert-top">ð´ \${a.title}</div>\`).join('')):bar.style.display='none';
    if(!alerts.length){document.getElementById('alertsList').innerHTML='<div class="success-box">â ××§××¤××× ×× × ×¨××× ×××!</div>';return;}
    document.getElementById('alertsList').innerHTML=alerts.map(a=>\`<div class="alert-card alert-sev-\${a.sev}"><div style="font-size:1.2rem;flex-shrink:0">\${a.icon}</div><div class="alert-body"><div class="alert-title">\${a.title}</div><div class="alert-desc">\${a.desc}</div></div>\${a.id?\`<button class="btn btn-sm btn-danger" onclick="toggle('\${a.id}','PAUSED','camp');setTimeout(runAlerts,1500)">â¸</button>\":''}</div>\`).join('');
  }catch(e){document.getElementById('alertsList').innerHTML=\`<div class="error-box">â ï¸ \${e.message}</div>\`;}
}
...
async function askAI(){
  ...
  el.style.display='block';el.textContent='â³ ×× ×ª×...';btn.disabled=true;
  try{
    if(!aiCtx)await prepAI();
    const r=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1000,system:\`××ª× ×× ×× ×§××¤××× ×× ×× ××¡× ×-Meta Ads. × ×ª×× × ××§××¤××× ××: \${aiCtx}. ×¢× × ××¢××¨××ª ×¢× ××××¦××ª ××¢×©×××ª.\`,messages:[{role:'user',content:q}]})});
    const d=await r.json();el.textContent=d.content?.[0]?.text||'××× ×ª×©×××';
  }catch(e){el.textContent='â ï¸ '+e.message;}
  btn.disabled=false;
}
function setQ(q){document.getElementById('aiQ').value=q;}

function exportCSV(){if(!campsData.length){alert('××¢× ×§××¤××× ×× ×§×××');return;}const h=['×©×','×¡××××¡','×××¨×','×ª×§×¦××','×××¦××','×××¤×¢××ª','×§×××§××','CTR'];const rows=campsData.map(c=>{const ins=c.insights?.data?.[0]||{};return[c.name,c.status,c.objective,c.daily_budget?c.daily_budget/100:'',ins.spend||0,ins.impressions||0,ins.clicks||0,ins.ctr||0].join(',');});const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\uFEFF'+[h.join(','),...rows].join('\n')],{type:'text/csv;charset=utf-8'}));a.download=\`meta-ads-\${today()}.csv\`;a.click();showExport('â CSV ×××¨×!');}
function exportJSON(){if(!campsData.length){alert('××¢× ×§××¤××× ×× ×§×××');return;}const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify({account:ACCT,date:today(),campaigns:campsData},null,2)],{type:'application/json'}));a.download=\`meta-ads-\${today()}.json\`;a.click();showExport('â JSON ×××¨×!');}
function exportPDF(){window.print();showExport('â ×¤×ª× ×××¤×¡×');}
async function copyShare(){if(!campsData.length){alert('××¢× ×§××¤××× ×× ×§×××');return;}const txt=\`××× Meta Ads - \${today()}\n\`+campsData.slice(0,5).map(c=>{const ins=c.insights?.data?.[0]||{};return\`\${c.name}: âª\${parseFloat(ins.spend||0).toFixed(0)} ×××¦××, \${parseFloat(ins.ctr||0).toFixed(2)}% CTR\`;}).join('\n');await navigator.clipboard.writeText(txt);showExport('â ×××¢×ª×§ â ××××§ ×××××¡××¤!');}
...
const server = http.createServer((req, res) => {
  ...
  if (pathname.startsWith('/api/meta/')) {
    const sid = req.headers['authorization'];
    if (!isValidSession(sid)) { res.writeHead(401); res.end(JSON.stringify({error:{message:'×× ×××××¨'}})); return; }
...
server.listen(CONFIG.PORT, () => {
  console.log(`â Meta Ads Pro Server running on port ${CONFIG.PORT}`);
  console.log(`ð¤ Username: ${CONFIG.USERNAME}`);
  console.log(`ð Password: ${CONFIG.PASSWORD}`);
  console.log(`ð Open: http://localhost:${CONFIG.PORT}`);
});
